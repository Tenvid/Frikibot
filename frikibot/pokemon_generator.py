...
def try_make_http_get(url: str, request_type: RequestTypes) -> requests.Response | None:
        ...
        try:
                ...
                raise requests.ConnectionError(
                        f"Status code not valid {response.status_code}"
                )
...
def _generate_embed(
        ...
        if sprite is None:
                embed.add_field(
                        ...
                        inline=False,
                        value="There is no sprite."
                        "That means that this case should not happen."
                        "Please, contact me on GitHub(@Tenvid) so I can fix this."
                        "Thank you :)",
                )
        return embed
